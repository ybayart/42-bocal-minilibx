...
{
  printf("Key up Win3 : %d\n",key);
  //  if (key==0xFF1B)
  if (key==53)
    mlx_destroy_window(mlx,win3);
  return (0);
}
int	key_win3_down(int key,void *p)
{
  printf("Key down Win3 : %d\n",key);
  //  if (key==0xFF1B)
  if (key==53)
    mlx_destroy_window(mlx,win3);
  return (0);
...
int	mouse_clic_win3(int button, int x,int y, void *p)
{
  printf("Mouse clic in Win3, at %dx%d.\n",x,y);
  return (0);
}
...
int	color_map_2(char *data,int bpp,int sl,int w,int h,int endian, int type)
{
  int	x;
  int	y;
  int	opp;
...
void	do_loop(void *toto)
{
  ...
  //mlx_string_put(mlx,win1,15,WIN1_SY/2,0xFFFFFF,"o");
  mlx_do_sync(mlx);
  printf("OK\n");
  sleep(2);
  ...
  printf("OK (xpm %dx%d)(img bpp2: %d, sizeline2: %d endian: %d type: %d)\n",
	 xpm1_x,xpm1_y,bpp2,sl2,endian2, -1); // ((t_img *)im2)->type);
  mlx_do_sync(mlx);
  sleep(2);
  ...
  mlx_mouse_hook(win3,mouse_clic_win3,0);

  mlx_hook(win3, 6 /* motion */, 0, mouse_win3, 0);
  mlx_hook(win3, 5 /* mouse up */, 0, mouse_clic_win3,0);
  mlx_hook(win3, 2 /* keydown */, 0, key_win3_down,0);
